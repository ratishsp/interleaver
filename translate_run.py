...
from tandem.translate import translate_scene, read_lines as _lines
...
class BackendDown(Exception):
    """Raised when STOP_AFTER_CONSECUTIVE_FAILS scenes fail in a row — unwinds the whole week loop."""
            ...
            for r in parse_storyboard(sb):
                ...
                for lang in a.langs:
                    ...
                    if not force and tgt.exists() and len(_lines(tgt)) == len(en_lines):
                        ...
                        continue
                    try:
                        out = translate_scene(client, model=a.model, wk=wk, stem=stem, lang=lang,
                                              en_lines=en_lines, da_lines=da_lines)
                        print(f"  [ok] {stem}.{lang}: {len(out)} lines")
                        wk_issues += check_scene_lang(client, model=a.model, wk=wk, stem=stem,
                                                      lang=lang, en_lines=en_lines, da_lines=da_lines,
