...
from gen_deck import (ROOT, DA_VOICE, DA_SPEED, CLIP_DIR, curriculum_fields,
...
def main(argv=None):
    ...
    ap.add_argument("--audio", action="store_true",
                    help="attach Danish audio (flashcards + the correct-answer voicing on eval backs)")
    ...
    for w in nums:
        ...
        sr = build_sr_deck(client, a.model, w, wdir, level=level, grammar=grammar, use_llm=True,
                           max_prod=a.max_prod, max_vocab=a.max_vocab, cache=cache, media=media)
        decks.append(sr)
        n_sr += len(sr.notes)
        if not a.no_eval:
    ...
    out_path = out_dir / f"{span}_all_da.apkg"
