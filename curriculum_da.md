...
- **Vocabulary — graded by FREQUENCY BAND, not a word count** *(revised 2026-06-26)*. The control is
  *which* words appear (common, level-appropriate), not *how many*: each level draws from a band of
  the most common Danish words — **A1 ≈ top 800, A2 ≈ top 1 500, B1 ≈ top 3 000, B2 ≈ top 5 000** —
  and the **verifier** checks each content word's frequency *rank* against the band (advisory). There
  is **no per-week word count and no cross-week cumulative tracking** — each week is self-contained;
  the old per-week New/Cumulative columns (a vestige of an enforced budget) have been dropped, and the
  generator carries no vocabulary list forward. Rationale: this is a comprehensible-input *listening*
  tool, so rich, repeated exposure to common words is *better* input than an artificially tiny palette.
  (Early weeks feel sparse mostly because of the *grammar* — only 3 verbs in week 1 — not the
...
| 45 | B2 | Media & advertising | Rhetorical structures; *ikke kun… men også* | Deconstructing a Danish ad campaign |
