...
- Level {level}, this week's grammar: {grammar} (earlier-week grammar may recur). The grammar need not surface in every scene — use it only where this scene naturally calls for it. Author natural {language} first — it may sit slightly above {level} where that's what's natural, but don't reach clearly beyond it.
- Tell it as Maya's own first-person account, in the present tense unless this week's grammar targets a past tense; attribute any quoted speech so it's clear who's speaking.
...
- One sentence per line in both arrays; let the scene run to a full ~15-20 line-pairs (a complete situation, not a sketch; if the story runs out sooner, stop rather than pad). The "{key}" and "en" arrays MUST have the same number of entries, aligned line-for-line.
